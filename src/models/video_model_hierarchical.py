...
from ..layers.layers import subpel_conv3x3, subpel_conv1x1, DepthConvBlock, \
    ResidualBlockWithStride, ResidualBlockUpsample, ConvLSTMCell
...
class MvEnc(nn.Module):
    ...
    def forward(self, x_s,x_d, context, quant_step):
        out = self.enc_1(torch.cat((x_s,x_d),dim=1))
        out = out * quant_step
        out = self.enc_2(out)
        if context is None:
...
class MvDec(nn.Module):
    def __init__(self, output_channel, channel, inplace=False):
        ...
        self.out_s = subpel_conv1x1(channel, output_channel, 2)
        self.out_d = subpel_conv1x1(channel, output_channel, 2)
...
class ContextHiddenFusion(nn.Module):
    def __init__(self, context_channel=g_ch_1x, hidden_channel=g_ch_1x):
        super().__init__()
        self.conv1 = nn.Conv2d(context_channel+hidden_channel, context_channel, 3, stride=1, padding=1)

    def forward(self, context, hidden):
        fuse_fea = self.conv1(torch.cat((context,hidden),dim=1))
        return fuse_fea
...
class DMC(CompressionModel):
    def __init__(self, anchor_num=4, ec_thread=False, stream_part=1, inplace=False):
        ...
        self.align_s = OffsetDiversity(inplace=inplace)
        self.align_d = OffsetDiversity(inplace=inplace)
        ...
        self.feature_adaptor2 = nn.Conv2d(g_ch_1x, g_ch_1x, 1)
        self.feature_adaptor3 = nn.Conv2d(g_ch_1x, g_ch_1x, 1)
        self.feature_extractor = FeatureExtractor(inplace=inplace)
        self.feature_extractor_hidden = FeatureExtractor(inplace=inplace)
        self.context_fusion_net = MultiScaleContextFusion(inplace=inplace)
        self.context_fusion_net_hidden = MultiScaleContextFusion(inplace=inplace)
        self.lstm = ConvLSTMCell(g_ch_1x, g_ch_1x, kernel_size=(3, 3), stride=1, bias=True)
        ...
        self.context_hidden_fusion1 = ContextHiddenFusion(g_ch_1x, g_ch_1x)
        self.context_hidden_fusion2 = ContextHiddenFusion(g_ch_2x, g_ch_2x)
        self.context_hidden_fusion3 = ContextHiddenFusion(g_ch_4x, g_ch_4x)
    ...
    def multi_scale_feature_extractor(self, dpb, index):
        feature_adaptor_list = [self.feature_adaptor, self.feature_adaptor2, self.feature_adaptor3]
        if dpb["ref_feature"] is None:
            feature = self.feature_adaptor_I(dpb["ref_frame"])
            next_hidden, next_cell = self.lstm(feature, (torch.zeros_like(feature), torch.zeros_like(feature)))
        else:
            index = index % 4
            index_map = [0, 1, 0, 2]
            index = index_map[index]
            feature = feature_adaptor_list[index](dpb["ref_feature"])
            next_hidden, next_cell = self.lstm(feature, (dpb["hidden"], dpb["cell"]))
        fea1, fea2, fea3 = self.feature_extractor(feature)
        return fea1, fea2, fea3, next_hidden, next_cell

    def hidden_fusion(self, warpframe, context1, context2, context3, dpb):
        if dpb["hidden"] is not None:
            h1, h2, h3 = self.feature_extractor_hidden(dpb["hidden"])
            c_h1, c_h2, c_h3 = self.context_fusion_net_hidden(h1, h2, h3)
            context1 = self.context_hidden_fusion1(context1, c_h1)
            context2 = self.context_hidden_fusion2(context2, c_h2)
            context3 = self.context_hidden_fusion3(context3, c_h3)
        return context1, context2, context3, warpframe

    def motion_compensation(self, dpb, ref_x_s, ref_x_d, mv_hat_s, mv_hat_d, index):
        warpframe_s = flow_warp(ref_x_s, mv_hat_s)
        warpframe_d = flow_warp(ref_x_d, mv_hat_d)
        warpframe = warpframe_s + warpframe_d
        mv2_s = bilineardownsacling(mv_hat_s) / 2
        mv3_s = bilineardownsacling(mv2_s) / 2
        mv2_d = bilineardownsacling(mv_hat_d) / 2
        mv3_d = bilineardownsacling(mv2_d) / 2
        ref_feature1, ref_feature2, ref_feature3, next_hidden, next_cell = self.multi_scale_feature_extractor(dpb, index)
        ref_feature1_s = bilinearupsacling(bilineardownsacling(ref_feature1))
        ref_feature2_s = bilinearupsacling(bilineardownsacling(ref_feature2))
        ref_feature3_s = bilinearupsacling(bilineardownsacling(ref_feature3))
        ref_feature1_d = ref_feature1 - ref_feature1_s
        ref_feature2_d = ref_feature2 - ref_feature2_s
        ref_feature3_d = ref_feature3 - ref_feature3_s
        context1_init_s = flow_warp(ref_feature1_s, mv_hat_s)
        context1_init_d = flow_warp(ref_feature1_d, mv_hat_d)
        context1_s = self.align_s(ref_feature1_s, torch.cat(
            (context1_init_s, warpframe_s, mv_hat_s), dim=1), mv_hat_s)
        context1_d = self.align_d(ref_feature1_d, torch.cat(
            (context1_init_d, warpframe_d, mv_hat_d), dim=1), mv_hat_d)
        context1 = context1_s + context1_d
        context2_s = flow_warp(ref_feature2_s, mv2_s)
        context2_d = flow_warp(ref_feature2_d, mv2_d)
        context2 = context2_s + context2_d
        context3_s = flow_warp(ref_feature3_s, mv3_s)
        context3_d = flow_warp(ref_feature3_d, mv3_d)
        context3 = context3_s + context3_d
        context1, context2, context3 = self.context_fusion_net(context1, context2, context3)
        context1, context2, context3, warpframe = self.hidden_fusion(warpframe, context1, context2, context3, dpb)
        return context1, context2, context3, warpframe, next_hidden, next_cell
    ...
    def motion_estimation_and_mv_encoding(self, x_s,x_d,ref_x_s,ref_x_d, dpb, mv_y_q_enc):
        est_mv_s = self.optic_flow(x_s, ref_x_s)
        est_mv_d = self.optic_flow(x_d, ref_x_d)
        mv_y = self.mv_encoder(est_mv_s, est_mv_d, dpb["ref_mv_feature"], mv_y_q_enc)
        return mv_y
    ...
    def compress(self, x, dpb, q_in_ckpt, q_index, frame_idx):
        x_s = bilinearupsacling(bilineardownsacling(x))
        x_d = x-x_s
        ref_x_s = bilinearupsacling(bilineardownsacling(dpb["ref_frame"]))
        ref_x_d = dpb["ref_frame"]-ref_x_s
        mv_y_q_enc, mv_y_q_dec, y_q_enc, y_q_dec = self.get_q_for_inference(q_in_ckpt, q_index)
        mv_y = self.motion_estimation_and_mv_encoding(x_s,x_d,ref_x_s,ref_x_d, dpb, mv_y_q_enc)
        ...
        mv_hat_s, mv_hat_d, mv_feature = self.mv_decoder(mv_y_hat, mv_y_q_dec)
        context1, context2, context3, _, next_hidden, next_cell = self.motion_compensation(dpb, ref_x_s, ref_x_d, mv_hat_s, mv_hat_d, frame_idx)
